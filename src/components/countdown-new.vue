...
export default {
  methods: {
    test () {
      ...
      h10.childNodes[1].innerHTML = h10.childNodes[3].innerHTML = '<span>' + Number(hh.substr(0, 1)) + '</span>'
      h0.childNodes[1].innerHTML = h0.childNodes[3].innerHTML = '<span>' + Number(String(hh).substr(1, 1)) + '</span>'

      m10.childNodes[1].innerHTML = m10.childNodes[3].innerHTML = '<span>' + Number(mm.substr(0, 1)) + '</span>'
      m0.childNodes[1].innerHTML = m0.childNodes[3].innerHTML = '<span>' + Number(mm.substr(1, 1)) + '</span>'

      s10.childNodes[1].innerHTML = s10.childNodes[3].innerHTML = '<span>' + Number(ss.substr(0, 1)) + '</span>'
      s0.childNodes[1].innerHTML = s0.childNodes[3].innerHTML = '<span>' + Number(ss.substr(1, 1)) + '</span>'
      ...
      function tick (mc, i, toZero = false) {
        // set numbers
        mc.childNodes[1].innerHTML = '<span>' + i + '</span>' // start upper
        mc.childNodes[2].innerHTML = '<span>' + i + '</span>' // start lower
        if (i === 9 || toZero) i = -1
        mc.childNodes[0].innerHTML = '<span>' + (i + 1) + '</span>' // end upper
        mc.childNodes[3].innerHTML = '<span>' + (i + 1) + '</span>' // end lower
        // animate tick
        TweenMax.fromTo(mc.childNodes[0], 0.3, { alpha: 0 }, { alpha: 1, ease: Power4.easeIn })
        TweenMax.fromTo(mc.childNodes[1], 0.3, { rotationX: 0, background: 'linear-gradient(0deg, rgba(200,200,200,1), rgba(255,255,255,1) 15%)' }, { rotationX: -90, ease: Power1.easeIn })
        TweenMax.fromTo(mc.childNodes[3], 0.5 + 0.2 * Math.random(), { rotationX: 90 }, { rotationX: 0, ease: Bounce.easeOut, delay: 0.3 })
        TweenMax.fromTo(mc.childNodes[2], 0.6, { alpha: 1 }, { alpha: 0, ease: Bounce.easeOut, delay: 0.3 })
      }
    }
  },
  mounted () {
